...
- 专门用于存储 **一串 信息**。
- 列表用 `[]` 定义，**数据** 之间使用 `,` 分隔。
- 列表的 **索引(又称下标)** 从 `0` 开始，即在 **列表** 中的位置编号。
...
- len(元组)获取元组的长度；元组.count(数据)统计数据在元组出现的次数。
- 元组[索引]获取对应位置的值；元组.index(数据)获取数据第一次出现的索引。
...
![img](./2_images/002_dectionary.png)
...
- 在 Python 中可以使用 **一对双引号** `"` 或者 **一对单引号** `'` 定义一个字符串
  - 虽然可以使用 `\"` 或者 `\'` 做字符串的转义，但是在实际开发中：
    ...
    - 如果字符串内部需要使用 `'`，可以使用 `"` 定义字符串
- 可用 **索引** 获取一个字符串中 **指定位置的字符**，索引从 **0** 开始
- 也可用 `for` **循环遍历** 字符串中每一个字符
